...
# ifdef __COMPILING_ADSK_CORE_NURBSCURVE2D_CPP__
# define ADSK_CORE_NURBSCURVE2D_API XI_EXPORT
# else
# define ADSK_CORE_NURBSCURVE2D_API
# endif
#else
# define ADSK_CORE_NURBSCURVE2D_API XI_IMPORT
#endif
...
public:
    ...
    static Ptr<NurbsCurve2D> createNonRational(const std::vector<Ptr<Point2D>>& controlPoints, int degree, const std::vector<double>& knots, bool isPeriodic);
    ...
    /// Returns the new NurbsCurve2D object or null if the creation failed.
    static Ptr<NurbsCurve2D> createRational(const std::vector<Ptr<Point2D>>& controlPoints, int degree, const std::vector<double>& knots, const std::vector<double>& weights, bool isPeriodic);
    ...
    std::vector<Ptr<Point2D>> controlPoints() const;
    ...
    bool getData(std::vector<Ptr<Point2D>>& controlPoints, int& degree, std::vector<double>& knots, bool& isRational, std::vector<double>& weights, bool& isPeriodic) const;
    ...
    /// end point that meet (with curvature continuity) forming a closed loop.
    /// Returns true if successful
    bool set(const std::vector<Ptr<Point2D>>& controlPoints, int degree, const std::vector<double>& knots, bool isRational, const std::vector<double>& weights, bool isPeriodic);
    ...
    Ptr<NurbsCurve2D> extract(double startParam, double endParam) const;
    ...
    /// Returns a new NurbsCurve2D object.
    Ptr<NurbsCurve2D> merge(const Ptr<NurbsCurve2D>& nurbsCurve) const;
    ...
    ADSK_CORE_NURBSCURVE2D_API static const char* classType();
    ADSK_CORE_NURBSCURVE2D_API const char* objectType() const override;
    ADSK_CORE_NURBSCURVE2D_API void* queryInterface(const char* id) const override;
    ADSK_CORE_NURBSCURVE2D_API static const char* interfaceId() { return classType(); }

private:

    // Raw interface
    ADSK_CORE_NURBSCURVE2D_API static NurbsCurve2D* createNonRational_raw(Point2D** controlPoints, size_t controlPoints_size, int degree, const double* knots, size_t knots_size, bool isPeriodic);
    ADSK_CORE_NURBSCURVE2D_API static NurbsCurve2D* createRational_raw(Point2D** controlPoints, size_t controlPoints_size, int degree, const double* knots, size_t knots_size, const double* weights, size_t weights_size, bool isPeriodic);
    virtual int controlPointCount_raw() const = 0;
    virtual NurbsCurve2D* copy_raw() const = 0;
    ...
    virtual Point2D** controlPoints_raw(size_t& return_size) const = 0;
    virtual double* knots_raw(size_t& return_size) const = 0;
    virtual bool getData_raw(Point2D**& controlPoints, size_t& controlPoints_size, int& degree, double*& knots, size_t& knots_size, bool& isRational, double*& weights, size_t& weights_size, bool& isPeriodic) const = 0;
    virtual bool set_raw(Point2D** controlPoints, size_t controlPoints_size, int degree, const double* knots, size_t knots_size, bool isRational, const double* weights, size_t weights_size, bool isPeriodic) = 0;
    virtual NurbsCurve2D* extract_raw(double startParam, double endParam) const = 0;
    virtual NurbsCurve2D* merge_raw(NurbsCurve2D* nurbsCurve) const = 0;
    virtual bool reverse_raw() = 0;
};

// Inline wrappers

inline Ptr<NurbsCurve2D> NurbsCurve2D::createNonRational(const std::vector<Ptr<Point2D>>& controlPoints, int degree, const std::vector<double>& knots, bool isPeriodic)
{
    Point2D** controlPoints_ = new Point2D*[controlPoints.size()];
    for(size_t i=0; i<controlPoints.size(); ++i)
        controlPoints_[i] = controlPoints[i].get();

    Ptr<NurbsCurve2D> res = createNonRational_raw(controlPoints_, controlPoints.size(), degree, knots.empty() ? nullptr : &knots[0], knots.size(), isPeriodic);
    delete[] controlPoints_;
    return res;
}

inline Ptr<NurbsCurve2D> NurbsCurve2D::createRational(const std::vector<Ptr<Point2D>>& controlPoints, int degree, const std::vector<double>& knots, const std::vector<double>& weights, bool isPeriodic)
{
    Point2D** controlPoints_ = new Point2D*[controlPoints.size()];
    for(size_t i=0; i<controlPoints.size(); ++i)
        controlPoints_[i] = controlPoints[i].get();

    Ptr<NurbsCurve2D> res = createRational_raw(controlPoints_, controlPoints.size(), degree, knots.empty() ? nullptr : &knots[0], knots.size(), weights.empty() ? nullptr : &weights[0], weights.size(), isPeriodic);
    delete[] controlPoints_;
    return res;
}

inline int NurbsCurve2D::controlPointCount() const
{
    int res = controlPointCount_raw();
    return res;
}

inline Ptr<NurbsCurve2D> NurbsCurve2D::copy() const
{
    Ptr<NurbsCurve2D> res = copy_raw();
    return res;
}

inline int NurbsCurve2D::degree() const
{
    int res = degree_raw();
    return res;
}

inline int NurbsCurve2D::knotCount() const
{
    int res = knotCount_raw();
    return res;
}

inline bool NurbsCurve2D::isRational() const
{
    bool res = isRational_raw();
    return res;
}

inline bool NurbsCurve2D::isClosed() const
{
    bool res = isClosed_raw();
    return res;
}

inline bool NurbsCurve2D::isPeriodic() const
{
    bool res = isPeriodic_raw();
    return res;
}

inline std::vector<Ptr<Point2D>> NurbsCurve2D::controlPoints() const
{
    std::vector<Ptr<Point2D>> res;
    size_t s;

    Point2D** p= controlPoints_raw(s);
    if(p)
    {
        res.assign(p, p+s);
        DeallocateArray(p);
...
inline std::vector<double> NurbsCurve2D::knots() const
{
...
inline bool NurbsCurve2D::getData(std::vector<Ptr<Point2D>>& controlPoints, int& degree, std::vector<double>& knots, bool& isRational, std::vector<double>& weights, bool& isPeriodic) const
{
    Point2D** controlPoints_ = nullptr;
...
inline bool NurbsCurve2D::set(const std::vector<Ptr<Point2D>>& controlPoints, int degree, const std::vector<double>& knots, bool isRational, const std::vector<double>& weights, bool isPeriodic)
{
    Point2D** controlPoints_ = new Point2D*[controlPoints.size()];
    for(size_t i=0; i<controlPoints.size(); ++i)
        controlPoints_[i] = controlPoints[i].get();
...
inline Ptr<NurbsCurve2D> NurbsCurve2D::extract(double startParam, double endParam) const
{
    Ptr<NurbsCurve2D> res = extract_raw(startParam, endParam);
    return res;
}

inline Ptr<NurbsCurve2D> NurbsCurve2D::merge(const Ptr<NurbsCurve2D>& nurbsCurve) const
{
    Ptr<NurbsCurve2D> res = merge_raw(nurbsCurve.get());
    return res;
}

inline bool NurbsCurve2D::reverse()
{
...
#undef ADSK_CORE_NURBSCURVE2D_API