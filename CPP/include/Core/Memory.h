...
public:
	...
	template<class V, class VPT>
	bool operator==(const Ptr<V, VPT>& rhs) const { return ptr_ == rhs.get(); }
...
public:
	...
	Iterator(const Ptr<Container, PT>& container, bool end = false) : container_(container), i_(0), end_(end) 
		{ if (!end && container_->count() == 0) end_ = true; assert(container_); }
	...
	typename Iterator::value_type& operator*()
	{
		...
		if (!end_ && !current_ && container_)
		{
			...
			assert(current_); // assume container is not poplulated with null values, so this should indicate a container error, check getLastError
		}
		return current_;
	}
	...
	typename Iterator::value_type& operator->() { return operator*(); }
